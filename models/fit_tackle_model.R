...
tackles = read.csv(paste0(data_dir, "tackles.csv"))

pass_tackle_info = pass_contact_info %>%
  filter(nflId != CarrierId, contact == 1) %>%
  left_join(tackles, by = join_by("gameId", "playId", "nflId")) %>%
  replace(is.na(.), 0)

run_tackle_info = run_contact_info %>%
  filter(nflId != CarrierId, contact == 1) %>%
  left_join(tackles, by = join_by("gameId", "playId", "nflId")) %>%
  replace(is.na(.), 0)
...
get_x_cont = function(data, type = "pass"){
  if (type == "pass"){
    X_cont = data %>% select(
      ...
      dist_f, s, a, yardsGained, airYards, Dist2side, Dist2endZ, Dist2LoS, Dist2FD, 
    ...
    X_cont = data %>% select(
      timeRemaining, defendersInTheBox, down,
      dist_f, s, a, yardsGained, Dist2side, Dist2endZ, Dist2LoS, Dist2FD, 
      rel_direction, bc_1, bc_2, bc_3, bc_4, bc_5, bc_6, bc_7, bc_8, bc_9, bc_10, bc_11,
      TeamInContact, weight, max_speed, pMaxSpeed, xSpeed, BC_xSpeed, Bc_weight
    )
  }
  ...
  X_cont$yardsGained = scales::rescale(X_cont$yardsGained, to = c(-1, 1), from = c(min(data$yardsGained), max(data$yardsGained)))
...
X_cont_pass = get_x_cont(pass_tackle_info, type = "pass")
...
X_cat_pass = get_x_cat(pass_tackle_info, bc_lvls = pass_bcId_idx, pos_lvls = pass_pos_idx)
...
# response
Y_train_pass = as.integer(pass_tackle_info$tackle)

pass_tackle_model = probit_flexBART(
...
save(pass_tackle_model, Y_train_pass, X_cont_pass, X_cat_pass, unif_cuts_pass, cutpoints_list_pass, cat_levels_list_pass, file = paste0(model_dir, "pass_tackle_model.RData"))

# run model

# inputs for continuous features
X_cont_run = get_x_cont(run_tackle_info, type = "run")
...
# inputs for categorical features
X_cat_run = get_x_cat(run_tackle_info, bc_lvls = run_bcId_idx, pos_lvls = run_pos_idx)
...
# response
Y_train_run = as.integer(run_tackle_info$tackle)

run_tackle_model = probit_flexBART(
  Y_train = Y_train_run,
...
# save model and inputs
save(run_tackle_model, Y_train_run, X_cont_run, X_cat_run, unif_cuts_run, cutpoints_list_run, cat_levels_list_run, file = paste0(model_dir, "run_tackle_model.RData"))

# predict for each row in pass_contact_info
...
pass_tackle_samples = predict_flexBART(
  fit = pass_tackle_model,
...
X_cat_test_run = get_x_cat(run_contact_info, bc_lvls = run_bcId_idx, pos_lvls = run_pos_idx)
run_tackle_samples = predict_flexBART(
  fit = run_tackle_model,
  X_cont = X_cont_test_run,
  X_cat = X_cat_test_run,
  verbose = TRUE
)
run_tackle_pred = apply(run_tackle_samples, MARGIN = 2, FUN = mean)
save(run_tackle_pred, file = paste0(data_dir, "run_tackle_pred.RData"))
